...
const X15 = ({ onClose }) => {
    ...
    const [isX15Open, setX15Open] = useState(false);
  ...
  const openX15 = useCallback(() => {
    setX15Open(true);
  }, []);

  const closeX15 = useCallback(() => {
    setX15Open(false);
  }, []);
  ...
  return (
    ...
    {isX15Open && (
        <PortalPopup
          ...
          placement="Centered"
          onOutsideClick={closeX15}
        >
          <Final onClose={closeX15} />
        </PortalPopup>
      )}
    </>
  );
