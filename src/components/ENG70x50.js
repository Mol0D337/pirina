...
const ENG70x50 = ({ onClose }) => {
    const navigate = useNavigate();
    const [isENG70x50Open, setENG70x50Open] = useState(false);
  ...
  const openENG70x50 = useCallback(() => {
    setENG70x50Open(true);
  }, []);

  const closeENG70x50 = useCallback(() => {
    setENG70x50Open(false);
  }, []);
  ...
  return (
    ...
    {isENG70x50Open && (
        <PortalPopup
          ...
          placement="Centered"
          onOutsideClick={closeENG70x50}
        >
          <Engfinal onClose={closeENG70x50} />
        </PortalPopup>
      )}
    </>
  );
