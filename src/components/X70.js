...
const X70 = ({ onClose }) => {
    ...
    const [isX70Open, setX70Open] = useState(false);
  ...
  const openX70 = useCallback(() => {
    setX70Open(true);
  }, []);

  const closeX70 = useCallback(() => {
    setX70Open(false);
  }, []);
  ...
  return (
    ...
    {isX70Open && (
        <PortalPopup
          ...
          placement="Centered"
          onOutsideClick={closeX70}
        >
          <Final onClose={closeX70} />
        </PortalPopup>
      )}
    </>
  );
